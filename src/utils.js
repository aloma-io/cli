  ...
  try {
    ...
    if (typeof tokenData === 'string' && tokenData.startsWith('id-')) {
      ...
      if (parsedToken.expires_at && parsedToken.expires_at < Date.now()) {
        throw new Error("Access token has expired. Please run `aloma auth` to re-authenticate.");
      }
    }
  } catch (parseError) {
    console.error(chalk.red('Token parse error:'), parseError.message);
    throw new Error(`Invalid token data stored: ${parseError.message}. Please run 'aloma auth' to re-authenticate.`);
  }
...
const transformTokenForGraphQL = async (accessToken, tokenData) => {
  try {
    ...
    const jwePayload = {
      ...
      selectedRealm: parsedUserData?.selectedRealm || await getSessionData('selectedWorkspace'),
    ...
    console.warn(chalk.yellow('Token transformation failed, using original:'), error);
    return accessToken;
  }
};
