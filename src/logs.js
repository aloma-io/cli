...
const LOG_PATTERNS = {
  ...
  'Customer Analytics': [
    ...
    { level: 'ERROR', message: 'Analytics calculation failed for customer: {id}' },
  ...
  'ML Model Training': [
    ...
    { level: 'ERROR', message: 'Training interrupted due to resource constraints' },
    { level: 'DEBUG', message: 'Model validation F1 score: {score}' },
    { level: 'SUCCESS', message: 'Model training completed. Accuracy: {accuracy}%' },
  ],
  default: [
...
function generateLogRecord(task) {
  ...
  let message = pattern.message
    ...
    .replace('{segment}', ['premium', 'standard', 'basic'][Math.floor(Math.random() * 3)])
    .replace('{period}', ['daily', 'weekly', 'monthly'][Math.floor(Math.random() * 3)])
    .replace('{orderNumber}', `ORD-${Math.floor(Math.random() * 100000)}`)
    .replace('{sku}', `SKU-${Math.floor(Math.random() * 10000)}`)
    .replace(
      '{errorCode}',
      ['AUTH_FAILED', 'INSUFFICIENT_FUNDS', 'TIMEOUT'][Math.floor(Math.random() * 3)]
    )
    .replace(
      '{reportType}',
      ['financial', 'performance', 'compliance'][Math.floor(Math.random() * 3)]
    )
    .replace('{section}', ['revenue', 'expenses', 'projections'][Math.floor(Math.random() * 3)])
    .replace('{chartType}', ['bar', 'line', 'pie'][Math.floor(Math.random() * 3)])
    .replace('{filename}', `report-${Math.floor(Math.random() * 1000)}.pdf`)
    .replace('{feature}', ['age', 'income', 'location'][Math.floor(Math.random() * 3)])
  ...
  message = message.replace(/\${x}/g, `$${(Math.random() * 500 + 50).toFixed(2)}`);
