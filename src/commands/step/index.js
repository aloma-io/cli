...
export async function addStep(name, workspaceIdentifier, nocodeType = null, filePath = null) {
  ...
  try {
    ...
    console.log(chalk.green(`Created step [${name.trim()}] with ID: ${stepId}`));
    ...
    if (filePath) {
      try {
        ...
        const conditionMatch = fileContent.match(/export const condition = ([\s\S]*?);/);
        const contentMatch = fileContent.match(/export const content = \(\) => \{([\s\S]*?)\};/);

        if (!conditionMatch || !contentMatch) {
          console.error(chalk.yellow('⚠ Could not parse condition and content from file. Using empty defaults.'));
          return;
        }
        ...
        try {
          ...
          console.error(chalk.red(`Invalid condition format: ${error.message}\nPlease ensure it is a valid JavaScript object.`));
          return;
        }
        ...
        if (updateData.saveAutomationStep) {
          console.log(chalk.green(`Successfully added step content from file: ${filePath}`));
        }
      } catch (error) {
        console.error(chalk.red(`Error reading or parsing file: ${error.message}`));
      }
    }
  } catch (error) {
...
export async function listSteps(includeDisabled = false, workspaceIdentifier) {
  ...
  try {
    ...
    steps.forEach((step) => {
      const status = step.enabled ? chalk.green('● Enabled') : chalk.red('● Disabled');
      const valid = step.valid ? chalk.green('✓ Valid') : chalk.red('✗ Invalid');
...
export async function showStep(stepId) {
  ...
  try {
    ...
    console.log(`${chalk.bold('Valid:')} ${step.valid ? chalk.green('Yes') : chalk.red('No')}`);
...
export async function deleteStep(stepId, workspaceIdentifier) {
  ...
  try {
    ...
    if (data.deleteAutomationStep) {
      ...
      console.log(chalk.yellow(`Step with ID ${stepId} not found or could not be deleted.`));
    }
  } catch (error) {
...
`;
    ...
    try {
      // Extract condition and content using regex
      const conditionMatch = modifiedContent.match(/export const condition = ([\s\S]*?);/);
      const contentMatch = modifiedContent.match(/export const content = \(\) => \{([\s\S]*?)\};/);
      ...
      console.error(chalk.red('Error parsing or updating step:'), error.message);
    }
...
export async function cloneStep(stepId, workspaceIdentifier) {
  ...
  try {
    ...
    console.log(chalk.green(`Successfully cloned step to workspace: ${workspaceId}`));
    return newStepId;
  } catch (error) {
