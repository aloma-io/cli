...
import { FIND_COMPANY_MEMBERS_QUERY, INVITE_TO_COMPANY_MUTATION, UPDATE_COMPANY_MEMBER_MUTATION } from './query.js';
...
export async function listUsers() {
  try {
    ...
    if (!user?.groups?.some(g => g === 'admin' || g === 'owner')) {
      console.log(chalk.red('Error: Only administrators or owners can list users'));
      return;
    }
    ...
    if (members.length === 0) {
      ...
      members.forEach((user) => {
        console.log(
          chalk.green(`- ${user.firstName || ''} ${user.lastName || ''} <${user.email}> (${user.id}) [${(user.groups || []).join(', ')}]`)
        );
      });
    }
    if (invitations.length > 0) {
      ...
      invitations.forEach((invite) => {
        console.log(
          chalk.yellow(`- ${invite.email} (invited at ${new Date(invite.createdAt).toLocaleString()})`)
        );
      });
    }
  } catch (error) {
...
export async function inviteUsers(emails, roles = 'developer') {
  try {
    const user = await getSessionData('user');
    if (!user?.groups?.some(g => g === 'admin' || g === 'owner')) {
      console.log(chalk.red('Error: Only administrators or owners can invite users'));
      return;
    }
    ...
    console.log(chalk.green(`Invited users: ${emailList.join(', ')} with roles: ${roleList.join(', ')}`));
  } catch (error) {
...
export async function updateUser(userId, roles) {
  try {
    const user = await getSessionData('user');
    if (!user?.groups?.some(g => g === 'admin' || g === 'owner')) {
      console.log(chalk.red('Error: Only administrators or owners can update users'));
      return;
    }
