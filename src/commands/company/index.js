...
export const listCompanies = async () => {
  ...
  realms.forEach((realm, index) => {
    console.log(`${index + 1}. ${realm.name} ${currentCompany === realm.id ? '[*]' : ''}`);
  });
};
...
export const switchCompany = async (identifier) => {
  try {
    ...
    const company = realms.find((realm) => realm.name === identifier || realm.id === identifier);
    if (!company) {
    ...
    if (cookies) {
      ...
      if (authCookie) {
        const token = authCookie.split(';')[0].split('=')[1].replace('Bearer%20', '');
